	...
	function check() {
		...
		if (api.isCurrentUserFree('com_poweradmin2')) {
			...
			if (!mask) {
				...
				api.Event.add('.pa-overlay-message > button', 'click', function() {
					api.showIntroduceProModal(jsn_joomlareset.dGsY8kMV, jsn_joomlareset.k8cuGn7t, 'com_poweradmin2', 'CTA Site Reset');
				});
			}
		}
		...
		// Otherwise, init site reset form.
			...
			api.Event.add('input[type="radio"]', 'click', function(event) {
				...
				var selected = event.target.parentNode.textContent.trim();

				while (section && section.nodeName != 'BODY') {
				...
				api.Tracking.postEvent(
					'Site Reset',
					'Reset Joomla',
					section.querySelector('h3').textContent.trim() + ': ' + selected
				);
			});
