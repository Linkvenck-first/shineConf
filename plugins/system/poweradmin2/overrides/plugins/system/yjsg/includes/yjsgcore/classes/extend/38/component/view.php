...
class JViewLegacy extends YjsgJViewLegacyDefault
{
	...
	public function loadTemplate($tpl = null)
	{
		...
		if ($yjsg->preplugin() && JFactory::getApplication()->isSite())
		{
			...
			$yjsg_path = array(
				YJSGTEMPLATEPATH . 'html' . YJDS . $option . YJDS . $folderName,
				YJSGPATH . 'legacy' . YJDS . 'html' . YJDS . $option . YJDS . $folderName
			);
		}
		else
		{
			...
			$yjsg_path = array(
				YJSGTEMPLATEPATH . 'html' . YJDS . $option . YJDS . $folderName,
				YJSGPATH . 'includes' . YJDS . 'html' . YJDS . $option . YJDS . $folderName
			);
		}
		...
		{
			JFactory::getApplication()->triggerEvent('onBeforeLoadTemplateFile',
				array(
					&$this->_path['template'],
					$filetofind
				));
